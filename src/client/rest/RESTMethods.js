...
class RESTMethods {
  ...
  followNewsChannel(channel, target) {
    return this.rest
      .makeRequest("post", Endpoints.Channel(channel).follow, true, {
        webhook_channel_id: target,
      })
      .then(
        (data) => client.channels.get(data.channel_id || "-1") || undefined
      );
  }
  ...
  addSlashCommand(data, guild) {
    return this.rest
      .makeRequest(
        "post",
        Endpoints.Commands(client.user.id, guild),
        true,
        data
      )
      .then(() => client);
  ...
  sendMessage(channel, content, options, files) {
  ...
  updateMessage(message, content, options) {
    options = options || {};
    var _options = options;
    ...
    return this.rest
      .makeRequest(
        ...
        true,
        {
          ...
          topic,
          type: type
            ? Constants.ChannelTypes[type.toUpperCase()]
  ...
  updateCurrentUser(_data, password) {
    ...
    return this.rest
      ...
      .then(
        (newData) => this.client.actions.UserUpdate.handle(newData).updated
      );
  }
  ...
  getGuildMember(guild, userID, cache) {
    return this.rest
      .makeRequest("get", Endpoints.Guild(guild).Member(userID), true)
      .then((data) => {
        if (cache)
  ...
  updateGuildMember(member, data, reason) {
    ...
    if (data.roles) {
      data.roles = [].concat(
        _toConsumableArray(
          new Set(
            data.roles.map(function (securityGroup) {
              return securityGroup instanceof Role
                ? securityGroup.id
                : securityGroup;
            })
          )
  ...
  createEmoji(guild, image, name, roles, reason) {
    ...
    return this.rest
      ...
      .then(
        (emoji) =>
          this.client.actions.GuildEmojiCreate.handle(guild, emoji).emoji
      );
  }
  ...
  editWebhook(webhook, options, reason) {
    ...
    return this.rest
      .makeRequest("patch", endpoint, auth, options, undefined, reason)
      .then((data) => {
  ...
  sendWebhookMessage(webhook, content, options, files) {
    options = options || {};
    if (files === undefined) files = null;
    var _options = options;
  ...
  addMessageReaction(message, emoji) {
    return this.rest
      .makeRequest(
        "put",
        Endpoints.Message(message).Reaction(emoji).User("@me"),
        true
      )
      .then(() =>
  ...
  acceptInvite(code) {
    ...
    return new Promise((resolve, reject) =>
      this.rest
        .makeRequest("post", Endpoints.Invite(code), true)
        .then((res) => {
          const handler = (guild) => {
            if (guild.id === res.id) {
              resolve(guild);
  ...
  syncIntegration(integration) {
    return this.rest.makeRequest(
      "post",
      Constants.Endpoints.Guild(integration.guild.id).Integration(
        integration.id
      ),
      true
    );
